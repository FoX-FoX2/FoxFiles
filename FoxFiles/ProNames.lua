...
if Manager(msg) then
if redis:get(DevProx.."Abs:Lock:ProNames"..msg.chat_id_) then
local Txt = text:match("^وضع توحيد (.*)$") or text:match("^ضع توحيد (.*)$")
send(msg.chat_id_, msg.id_,'⌁︙تم تعيين ↫ '..Txt..' كتوحيد للمجموعه')
redis:set(DevProx.."Abs:ProNames:Txt"..msg.chat_id_,Txt)
else
...
redis:set(DevProx.."Abs:ProNames:Num"..msg.chat_id_,Num)
else
send(msg.chat_id_, msg.id_,'⌁︙ميزة التوحيد معطله يرجى تفعيلها')
end
end
...
if redis:get(DevProx.."Abs:ProNames:Txt"..msg.chat_id_) then
local ProNamesTxt = redis:get(DevProx.."Abs:ProNames:Txt"..msg.chat_id_)
local ProNamesNum = redis:get(DevProx.."Abs:ProNames:Num"..msg.chat_id_) or 5
...
if redis:get(DevProx.."Abs:Lock:ProNames"..msg.chat_id_) and redis:get(DevProx.."Abs:ProNames:Txt"..msg.chat_id_) then 
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
if result and result.first_name_ then 
if result.first_name_:match("(.*)"..redis:get(DevProx.."Abs:ProNames:Txt"..msg.chat_id_).."(.*)") then 
redis:srem(DevProx..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_)
else
local ProNamesTxt = redis:get(DevProx.."Abs:ProNames:Num"..msg.chat_id_) or 5
local UserNum = redis:get(DevProx.."Abs:ProNames:UserNum"..msg.chat_id_..msg.sender_user_id_) or 0
if (tonumber(UserNum) == tonumber(ProNamesTxt) or tonumber(UserNum) > tonumber(ProNamesTxt)) then 
redis:sadd(DevProx..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_)
redis:del(DevProx.."Abs:ProNames:UserNum"..msg.chat_id_..msg.sender_user_id_)
...
redis:set(DevProx.."Abs:Lock:ProNames"..msg.chat_id_,true)
end
if text == "تعطيل التوحيد" and Constructor(msg) then
send(msg.chat_id_, msg.id_, '⌁︙تم تعطيل توحيد المجموعه')
redis:del(DevProx.."Abs:Lock:ProNames"..msg.chat_id_)
end
end
